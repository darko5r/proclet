...
fn tweak_env_for_uid(env_pairs: &mut Vec<(String, String)>, uid: u32) {
    ...
    if !parent_has("XDG_CONFIG_HOME") && !env_pairs.iter().any(|(k, _)| k == "XDG_CONFIG_HOME") {
        env_pairs.push(("XDG_CONFIG_HOME".to_string(), format!("{home}/.config")));
    }

    if !parent_has("XDG_CACHE_HOME") && !env_pairs.iter().any(|(k, _)| k == "XDG_CACHE_HOME") {
        env_pairs.push(("XDG_CACHE_HOME".to_string(), format!("{home}/.cache")));
    }

    // IMPORTANT: do NOT touch XDG_RUNTIME_DIR here.
    // gui.rs will decide the correct strategy (attach vs bridge) later.

    if !parent_has("XAUTHORITY") && !env_pairs.iter().any(|(k, _)| k == "XAUTHORITY") {
        env_pairs.push(("XAUTHORITY".to_string(), String::new()));
    }

    if uid != 0 && !env_pairs.iter().any(|(k, _)| k == "XDG_DATA_DIRS") {
...
fn print_summary(cli: &Cli, use_user: bool, use_pid: bool, use_mnt: bool, use_net: bool) {
    ...
    eprintln!(
        "  {} {}",
        label("ns"),
        format!("user={} pid={} mnt={} net={}", use_user, use_pid, use_mnt, use_net)
    );
...
fn main() {
    ...
    let (new_root_path, auto_root_for_cleanup): (Option<PathBuf>, Option<PathBuf>) =
        if let Some(explicit) = &cli.new_root {
            ...
            let path_str = unsafe { CStr::from_ptr(res) }.to_string_lossy().into_owned();
    ...
    // Validate --as-user with userns (your existing restriction)
    if let Some(target_uid) = cli.as_user {
        if use_user && euid != 0 && target_uid != euid {
            print_error(
    ...
    // Smart GUI mode (root + GUI browser, no explicit --as-user)
    if drop_uid.is_none()
        && !cli.cursed
        && !cli.cursed_host
        ...
        && euid == 0
    {
        if let Some(first) = cli.cmd.first() {
            ...
            if GUI_BROWSERS.iter().any(|&name| name == argv0) {
                ...
                if use_user {
                    use_user = false; // pid+mnt only for GUI case
                }
    ...
    // --- command vector (mutable, can be wrapped) ---
    let mut cmd_vec = cli.cmd.clone();

    // If we’re dropping to a user, make HOME/USER sane first.
    ...
    // Desktop prep (the important part): do this BEFORE cstrings().
    // This is where we attach/bridge wayland, scrub dbus, optionally wrap with dbus-run-session,
    // and return extra bind mounts needed for the desktop emulation.
    let mut extra_binds: Vec<(PathBuf, PathBuf, bool)> = Vec::new();
    if euid == 0 {
        if let Some(uid) = drop_uid {
            if uid != 0 && !cli.cursed && !cli.cursed_host {
                extra_binds = proclet::gui::prepare_desktop(
                    &mut env_pairs,
                    &mut cmd_vec,
                    uid,
                    verbosity,
    ...
    // Build CString argv LAST (after cmd_vec mutations)
    let cmd_slices: Vec<&str> = cmd_vec.iter().map(|s| s.as_str()).collect();
    ...
    // GPU shim is opt-in via env var
    ...
    let mut opts = ProcletOpts {
        ...
        binds: {
            let mut b = parse_binds(&cli.bind);
            b.extend(extra_binds);
            b
        },
